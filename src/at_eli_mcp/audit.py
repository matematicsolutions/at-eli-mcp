...
Every call to every MCP tool writes one JSON line to:
    ...
    ~/.matematic/audit/at-eli-mcp.jsonl

(or to the directory given by ``AT_ELI_AUDIT_DIR``).

If the write fails, the tool returns an error instead of silently continuing
(Art. 2 CONSTITUTION.md). Inherited verbatim from de-eli-mcp.
"""
...
_AUDIT_FILENAME = "at-eli-mcp.jsonl"


def _resolve_audit_dir() -> Path:
    env = os.environ.get("AT_ELI_AUDIT_DIR")
    if env:
