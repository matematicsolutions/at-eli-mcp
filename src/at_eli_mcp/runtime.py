...
Pulls a tiny static ``at-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/at-runtime.json
...
Opt-out: set ``AT_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "AT_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/at-eli-mcp/releases/latest/download/at-runtime.json.gz",
)
_CACHE_NAME = "at-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "at-eli-mcp-runtime (+https://github.com/matematicsolutions/at-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("AT_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "at-eli"
